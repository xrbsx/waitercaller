...
class MockDBHelper:
    ...
    def add_request(self, table_id, time):
        import pdb; pdb.set_trace()
        owner = self.get_ownertable(table_id)
        tables = self.get_tables(owner)
        MOCK_REQUESTS.append({"_id": table_id, "owner": tables["owner"], "table_number": tables["number"], "table_id": table_id, "time": time})
        return True

    def get_ownertable(self, table_id):
        for i, table in enumerate(MOCK_TABLES):
            if table.get("_id") == table_id:
                return table.get("owner")
